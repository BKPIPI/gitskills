...
function Snake() {
    ...
    this.directionNum = { //存储蛇走的方向，用一个对象来表示
        left: {
            x: -1,
            y: 0,
            rotate: 180 //蛇头在不同的方向中应该进行旋转，要不始终是在向右
        },
        right: {
            x: 1,
            y: 0,
...
Snake.prototype.getNaxtPos = function() {
    ...
    //下一个点是食物，吃
    if(fruits && fruits.pos[0] == nextPos[0] && fruits.pos[1] == nextPos[1]) {
        console.log('撞到食物了！');
...
Snake.prototype.strategies = {
    move: function(format) { //这个参数用于决定要不要删除最后一个方块（蛇尾）。当传了这个参数后就表示要做的事情是吃
        ...
        if(!format) {
            this.tail.remove(); //如果format的值为false，表示需要删除（除了吃之外的操作）
...
function createFruits() {
    ...
    var include = true; //循环跳出的条件，true表示食物的坐标在蛇身上（需要继续循环）。false表示食物的坐标不再蛇的身上（不循环了）
    while(include) {
    ...
    //生成食物
    fruits = new Square(x, y, 'fruits');
    fruits.pos = [x, y]; //存储一下生成食物的坐标，用于跟蛇头要走的下一个点做对比
    var fruitsDom = document.querySelector('.fruits');
    if(fruitsDom) {
        fruitsDom.style.left = x*sw+'px';
        fruitsDom.style.top = y*sh+'px';
    }else{
        fruits.create();
    }
}
